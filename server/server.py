...
In this form after the server establishes connection and receives the
information from the client the server is expected to run the web scraper
and return the appropriate prices for the requested game.
The search is all connected to the steam search.
It will return the value for the game that is closest to the user search.
In the odd case that no such games are found the server
...
def handle_connection(conn) -> None:
    ...
    while True:
        ...
        if len(game_names_found) > 0:
            ...
            combined_prices = soup.find_all("div", class_="discount_prices")[0]
            all_divs_arr = combined_prices.find_all("div")
            ...
            if len(all_divs_arr) == 2:
                ...
                # If the game has one div then is either free or not discounted
...
def main() -> None:
    ...
    while True:
        conn = None
        try:
            # Create a connection when a client requests on a different thread
